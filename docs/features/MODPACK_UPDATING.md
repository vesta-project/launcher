...
An update waits in a cancellable Task Manager readiness phase until Minecraft for that instance exits. Readiness holds the shared per-instance launch/update conflict lock but does not consume a global worker permit, so unrelated downloads and imports can still start. The task explains that closing Minecraft will continue the update; the user can cancel instead. Cancellation or discard before `run` restores installation status and clears the pending marker through task abandonment cleanup, without planning or mutating files. Launch and update share the same conflict key; launch reloads authoritative Instance status under that exclusion so a stale frontend snapshot cannot start Java during an update. After a worker permit is acquired, the update revalidates process state before planning and again before live mutation. If a process appears after the audit, the plan is discarded, the task waits again, and planning recomputes. The watcher remains attached while waiting, and planning and file changes begin only after exit. Java process matching uses the exact game directory in a working directory or command argument, including explicit `--gameDir=...` and split `--gameDir /path` pairing, plus exact path tokens for launchers that omit the flag; another Minecraft instance does not delay this update. This is an in-session queue: restarting Vesta clears an unapplied pending update through normal recovery. The in-app guard cannot prevent an external Java process from launching against the same game directory after the final revalidation; that remaining OS boundary is accepted and documented here.
...
The current-file content audit remains necessary for three-way user-edit protection and logs its path count and duration. Conflict selection no longer requests provider release rankings. Watcher bursts have a bounded debounce window; queue overflow triggers a reconciliation scan that also cleans deleted instance resource roots. Each watcher registration has an identity so replacement/unwatch waits until the previous worker can no longer publish stale rows. Enrichment validates prepared candidates on the blocking pool with bounded concurrency, follows verified enabled/disabled renames, and skips disappeared or changed files, preventing stale prepared paths from recreating missing rows.

### Disabled Paths
When the current tracked mod exists only at its `.disabled` path, replacement journals both that physical path and the new enabled destination. Commit leaves the selected pack file enabled; rollback restores the previous disabled file. Removal also resolves the physical disabled path and retains hash-based protection for user edits. Existing ambiguous enabled/disabled pairs are preserved rather than overwritten by enable/disable operations. Pack-switch duplicate resolution applies enablement changes through one Ledger batch Interface: destinations are preflighted, losers are disabled before winners are enabled, row facts commit in one database transaction, and completed filesystem renames are compensated in reverse order if that write fails. A database transaction alone is not treated as atomic filesystem/database publication; failed compensation retains recovery path information. Derived Ledger reconciliation failure after a committed pack version does not roll the pack version back.
