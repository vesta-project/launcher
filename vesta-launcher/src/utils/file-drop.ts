...
class DropZoneManager {
  ...
  private overlayChannelListener: ((event: MessageEvent<OverlayBridgeMessage>) => void) | null = null;
  ...
  cleanup(): void {
    if (this.broadcastChannel) {
      try {
        if (this.overlayChannelListener) {
          this.broadcastChannel.removeEventListener("message", this.overlayChannelListener);
        }
    ...
    this.zones.forEach(({ element }) => element.classList.remove("drop-zone--active"));
    this.zones.clear();
  ...
  registerZone(
    ...
    console.log("[FileDrop] Registered zone:", element.dataset.dropZoneId, "total zones:", this.zones.size);
  ...
  unregisterZone(element: HTMLElement): void {
    ...
    this.logDebug("Unregistered drop zone", element.dataset.dropZoneId ?? "unknown");
  }

  private async attachDragDropListener(): Promise<void> {
  ...
  private async showOverlayForDrag(): Promise<void> {
    ...
    try {
      ...
      if (!this.overlayVisible) {
        ...
        console.log("[FileDrop] Overlay shown for drag detection (DEBUG VISIBLE)");
      }
    } catch (error) {
  ...
  private handleHover(position: PhysicalPosition | null): void {
    ...
    console.log("[FileDrop] Found zone element:", zoneElement?.dataset?.dropZoneId || "none");
    if (!zoneElement) {
      this.clearActiveZone();
      return;
    }
  ...
  private handleDrop(paths: string[], position: PhysicalPosition | null): void {
    console.log("[FileDrop] handleDrop called with", paths.length, "paths:", paths);
    ...
    if (!zoneElement && !position && this.zones.size > 0) {
      console.log("[FileDrop] No active zone or position, using first registered zone");
      zoneElement = Array.from(this.zones.keys())[0] ?? null;
    }
    
    if (!zoneElement && position) {
      zoneElement = this.resolveZoneFromPosition(position);
      console.log("[FileDrop] Resolved zone from position:", zoneElement?.dataset.dropZoneId ?? "none");
    } else if (zoneElement) {
      console.log("[FileDrop] Using active zone:", zoneElement?.dataset.dropZoneId ?? "none");
    }
    ...
    console.log("[FileDrop] Filtered", paths.length, "paths to", filtered.length, "based on zone options");
    ...
    console.log("[FileDrop] Dispatching drop to zone:", zoneElement.dataset.dropZoneId, "with", filtered.length, "files");
    ...
    try {
      ...
      console.log("[FileDrop] File(s) dropped successfully:", filtered.length, "files in zone", zoneElement.dataset.dropZoneId);
    } catch (error) {
  ...
  private handleLeave(): void {
    if (this.activeZone) {
      this.logDebug("Drag left active zone", this.activeZone.dataset.dropZoneId);
    }
    this.clearActiveZone();
    this.broadcastHover("hover-end", null);
  }

  private resolveZoneFromPosition(position: PhysicalPosition): HTMLElement | null {
  ...
  private filterPaths(paths: string[], options: NormalizedDropZoneOptions): string[] {
    console.log("[FileDrop] Filtering", paths.length, "paths with options:", options);
    const filtered = paths.filter((path) => {
      ...
      if (options.accept === "files" && directory) {
        console.log("[FileDrop] Rejected (zone accepts files only, got directory)");
        return false;
      }
      if (options.accept === "folders" && !directory) {
        console.log("[FileDrop] Rejected (zone accepts folders only, got file)");
        return false;
      }
    ...
    console.log("[FileDrop] Filter result:", filtered.length, "of", paths.length, "paths accepted");
    return filtered;
  }
  ...
  private broadcastHover(type: BroadcastMessage["type"], element: HTMLElement | null): void {
    if (!this.broadcastChannel) {
      return;
    }
  ...
  private attachOverlayChannelListener(): void {
    ...
    this.overlayChannelListener = (event: MessageEvent<OverlayBridgeMessage>) => {
      console.log("[FileDrop] Received broadcast message:", event.data?.type);
      this.handleOverlayBridgeMessage(event.data);
    };
    this.broadcastChannel.addEventListener("message", this.overlayChannelListener);
  }

  private handleOverlayBridgeMessage(message: OverlayBridgeMessage | null | undefined): void {
    if (!message || message.source !== "overlay") {
      return;
    }
    console.log("[FileDrop] Received overlay message:", message.type, "at position:", message.position);
    switch (message.type) {
      case "overlay-enter":
      case "overlay-over":
      ...
      case "overlay-html-drop":
        void this.handleOverlayDrop(message.paths ?? [], message.position ?? null);
        break;
      default:
        break;
    }
  ...
  private async handleOverlayHover(position: OverlayPosition | null): Promise<void> {
  ...
  private async handleOverlayDrop(paths: string[], position: OverlayPosition | null): Promise<void> {
    const physical = await this.convertOverlayPosition(position);
    this.handleDrop(paths, physical);
  }

  private async convertOverlayPosition(position: OverlayPosition | null): Promise<PhysicalPosition | null> {
    if (!position) {
      return null;
    }
  ...
  private async syncOverlayWithZone(element: HTMLElement): Promise<void> {
    console.log("[FileDrop] syncOverlayWithZone called for element:", element.dataset.dropZoneId);
    if (!this.overlayReady) {
      await this.ensureOverlayReady();
    }
    if (!this.overlayReady) {
    ...
    try {
      ...
      console.log("[FileDrop] Drop window positioned at:", { x, y, width, height });
  ...
  private resolveOverlayVisualState(element: HTMLElement): OverlayVisualState {
    ...
    const outlineWidth = computed.outlineWidth && computed.outlineWidth !== "0px" ? computed.outlineWidth : "2px";
    const outlineStyle = computed.outlineStyle && computed.outlineStyle !== "none" ? computed.outlineStyle : "dashed";
