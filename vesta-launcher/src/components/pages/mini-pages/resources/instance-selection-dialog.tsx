...
const InstanceSelectionDialog: Component<InstanceSelectionDialogProps> = (props) => {
    const [installedMap, setInstalledMap] = createSignal<Record<number, {id: string, name: string, type: string, versionId: string, hash: string | null}[]>>({});
    ...
    const sortedInstances = () => [...instancesState.instances].sort((a, b) => {
            ...
            await Promise.all(instances.map(async (inst) => {
                try {
                    ...
                    if (inst.id === resources.state.selectedInstanceId) {
                        ...
                        const installed = await invoke<any[]>("get_installed_resources", { instanceId: inst.id });
    ...
    const getCompatibility = (instance: Instance) => {
        ...
        if (resType === 'mod') {
            ...
            if (hasFabric || hasForge || hasQuilt || hasNeoForge) {
                if (instLoader === 'fabric' && !hasFabric) 
                    return { type: 'incompatible', reason: "This mod is not compatible with Fabric." };
                if (instLoader === 'forge' && !hasForge) 
                    return { type: 'incompatible', reason: "This mod is not compatible with Forge." };
                if (instLoader === 'neoforge' && !hasNeoForge) 
                    return { type: 'incompatible', reason: "This mod is not compatible with NeoForge." };
                if (instLoader === 'quilt' && !hasQuilt && !hasFabric) 
                    return { type: 'incompatible', reason: "This mod is not compatible with Quilt." };
            }
        }

        // 3. Version-based check (the "truth")
        const versionsToUse = (props.versions && props.versions.length > 0) ? props.versions : fetchedVersions();
        ...
        if (isLoadingVersions()) {
            return { type: 'incompatible' as const, reason: "Loading compatibility data..." };
        }

        // If we finished loading and versions list is STILL empty
        if (versionsToUse.length === 0) {
            if (resType === 'mod' || resType === 'shader') {
                return { type: 'incompatible' as const, reason: "No compatible versions found." };
            }
        }
        ...
        <Dialog open={props.isOpen} onOpenChange={(open) => !open && props.onClose()}>
            <DialogContent class="instance-selection-dialog">
                <DialogHeader>
                    ...
                    <p>Choose where to install {props.project?.name || "this resource"}.</p>
                </DialogHeader>
                        ...
                        {(instance) => {
                            ...
                            return (
                                <button 
                                    ...
                                    title={isUpdateAvailable() ? "Update available for this instance" : (isAlreadyInstalled() ? "Already installed in this instance" : (isDisabled() ? comp().reason : ""))}
                                >
