  ...
  createEffect(() => {
    ...
    batch(() => {
      ...
      if (d) {
        ...
        if (!isHooksDirty()) {
          if (d.useGlobalHooks !== undefined)
            setUseGlobalHooks(d.useGlobalHooks);
          if (d.preLaunchHook !== undefined)
            setPreLaunchHook(d.preLaunchHook || "");
          if (d.wrapperCommand !== undefined)
            setWrapperCommand(d.wrapperCommand || "");
          if (d.postExitHook !== undefined)
            setPostExitHook(d.postExitHook || "");
        }
      }
  ...
  createEffect(() => {
    ...
    if (info && normalizedIsModpack()) {
      batch(() => {
        ...
        if (!isMemoryDirty()) {
          const rec = calculateRecommendedMemory(
            totalRam(),
            info.modCount ?? 0,
            info.recommendedRamMb,
          );
                  ...
                  <div class={styles["form-section"]}>
                    ...
                    <div class={styles["memory-setting"]}>
                      <div class={styles["memory-header"]}>
                        ...
                        >
                          {formatMemory(memory()[0])} {memoryUnit()} —{" "}
