...
function SettingsPage(props: { close?: () => void; router?: MiniRouter }) {
	...
	const javaOptions = createMemo(() => {
		...
		reqs.forEach((req: any) => {
			const allForVersion = globalPathsData.filter((p: any) => p.major_version === req.major_version);
			...
			const managed = allForVersion.find((p: any) => p.is_managed && p.is_active);
			...
			detectedJavas
				...
				.forEach((det: any) => {
					options.push({
						...
						path: det.path,
						isActive: current?.path === det.path && current?.is_active && !current?.is_managed,
			...
			for (const p of allForVersion) {
				...
				if (detectedJavas.some((d: any) => d.path === p.path && d.major_version === req.major_version))
					continue;
				options.push({
					type: "custom",
					version: req.major_version,
	...
	const handleOpenRuntimeStorageLocation = async () => {
		if (hasTauriRuntime()) {
					...
					<TabsContent class={styles["tabs-content"]} value="general">
						...
						>
							<GeneralSettingsTab
								...
								navigateToImporter={() => activeRouter()?.navigate("/install/import")}
							/>
						</Suspense>
					</TabsContent>
