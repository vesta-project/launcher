...
export function getInstancePrimaryAction(state: PrimaryActionState) {
	...
	if (state.operationInProgress) {
		return {
			label: `${state.operationLabel || "Working"}…`,
			icon: "spinner" as const,
			tone: "primary" as const,
		};
	}
	if (state.updateRecovery)
		return {
	...
	if (state.interrupted) {
		const operation =
			state.lastOperation === "hard-reset"
				? "reset"
...
export function summarizeResources(
	resources: Array<{ source_kind?: string | null }>,
	knownUpdates?: number,
) {
	const bundled = resources.filter(
		(resource) => resource.source_kind?.toLowerCase() === "modpack",
	).length;
	const custom = resources.length - bundled;
	const parts = [`${resources.length} installed`];
	if (bundled > 0 && custom > 0)
		parts.push(`${bundled} bundled · ${custom} custom`);
	if (knownUpdates !== undefined && knownUpdates > 0)
		parts.push(`${knownUpdates} update${knownUpdates === 1 ? "" : "s"}`);
	return parts.join(" · ");
}
