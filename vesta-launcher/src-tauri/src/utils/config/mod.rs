...
pub fn initialize_config_db() -> Result<SQLiteDB, anyhow::Error> {
    ...
    conn.execute(
        &format!("INSERT OR IGNORE INTO {} (id) VALUES (1)", AppConfig::name()),
        []
    )?;
...
mod tests {
    ...
    fn initialize_test_db() -> SQLiteDB {
        ...
        let conn = db.get_connection();
        conn.execute("INSERT OR IGNORE INTO app_config (id) VALUES (1)", []).unwrap();
    ...
    fn test_config_update() {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, background_hue, theme FROM app_config WHERE id = 1"
        ).unwrap();
        
        let (id, hue, theme): (i32, i32, String) = stmt.query_row([], |row| {
