...
impl ResourceSource for ModrinthSource {
    async fn search(&self, query: SearchQuery) -> Result<SearchResponse> {
        ...
        let response: ModrinthSearchResult = self.client.get(&url)
            .send()
            .await?
            .json()
            .await?;

        let hits = response.hits.into_iter().map(|hit| ResourceProject {
            id: hit.project_id,
    ...
    async fn get_project(&self, id: &str) -> Result<ResourceProject> {
        ...
        let project: ModrinthProject = self.client.get(&url)
            .send()
            .await?
            .json()
            .await?;
        ...
        let members: Vec<ModrinthTeamMember> = self.client.get(&team_url)
            .send()
            .await?
            .json()
            .await?;
    ...
    async fn get_versions(&self, project_id: &str) -> Result<Vec<ResourceVersion>> {
        ...
        let versions: Vec<ModrinthVersion> = self.client.get(&url)
            .send()
            .await?
            .json()
            .await?;
