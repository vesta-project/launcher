...
async fn test_modrinth_search() {
    ...
    assert!(
        ...
        "No hits found for Modrinth search"
    );

    let project = &response.hits[0];
    assert_eq!(project.name, "Abandoned Prismarine Tower");
    assert!(!project.author.is_empty(), "Author should not be empty");
    assert!(
        project.published_at.is_some(),
        "Published date should be present"
    );
}
...
async fn test_curseforge_search() {
    ...
    assert!(
        !response.hits.is_empty(),
        "No hits found for CurseForge search"
    );
...
async fn test_modrinth_versions() {
    ...
    let versions = source
        .get_versions("abandoned-prismarine-tower", None, None)
        .await;
    assert!(
        versions.is_ok(),
        "Modrinth get_versions failed: {:?}",
        versions.err()
    );
    let versions = versions.unwrap();
    assert!(
        !versions.is_empty(),
        "No versions found for Modrinth project"
    );

    for version in versions {
        assert!(
            version.download_url.starts_with("https://"),
            "Download URL should be absolute: {}",
...
async fn test_curseforge_versions() {
    ...
    assert!(
        search_result.is_ok(),
        "CurseForge search in versions test failed"
    );
    let hits = search_result.unwrap().hits;
    assert!(
        !hits.is_empty(),
        "Could not find Mouse Tweaks to test versions"
    );
    ...
    assert!(
        versions.is_ok(),
        "CurseForge get_versions failed for ID {}: {:?}",
        project_id,
        versions.err()
    );
    let versions = versions.unwrap();
    assert!(
        !versions.is_empty(),
        "No versions found for CurseForge project {}",
        project_id
    );
    ...
    for version in versions {
        if !version.download_url.is_empty() {
            assert!(
                version.download_url.starts_with("http"),
                "Download URL should be absolute: {}",
                version.download_url
            );
        }
