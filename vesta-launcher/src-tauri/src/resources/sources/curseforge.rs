...
fn get_deobfuscated_key() -> String {
    CURSEFORGE_API_KEY_OBFUSCATED
        .iter()
        .map(|b| (b ^ CURSEFORGE_SEED) as char)
        .collect()
}
...
impl CurseForgeSource {
    pub fn new() -> Self {
        ...
        log::info!(
            "CurseForgeSource initializing with key (len: {})",
            key.len()
        );
    ...
    pub async fn fetch_categories_direct(&self) -> Result<Vec<CFCategoryFull>> {
        ...
        if !status.is_success() {
            ...
            return Err(anyhow!(
                "CurseForge API error fetching categories ({}): {}",
                status,
                body
            ));
        }
        ...
        match serde_json::from_str::<CFAllCategoriesResponse>(&body) {
            ...
            Err(e) => {
                log::error!(
                    "Failed to decode CurseForge categories JSON: {}. Body length: {}",
                    e,
                    body.len()
                );
...
impl ResourceSource for CurseForgeSource {
    async fn get_categories(&self) -> Result<Vec<ResourceCategory>> {
        ...
        Ok(categories
            ...
            .filter(|c| {
                ...
                if c.class_id == Some(6945)
                    && (c.id == 4550
                        || c.id == 4547
                        || c.id == 4552
                        || c.parent_category_id == Some(4550)
                        || c.parent_category_id == Some(4547)
                        || c.parent_category_id == Some(4552))
                {
                    return false;
                }
    ...
    async fn search(&self, query: SearchQuery) -> Result<SearchResponse> {
        ...
        let mut url = format!(
            "https://api.curseforge.com/v1/mods/search?gameId=432&classId={}&index={}&pageSize={}",
            class_id, query.offset, query.limit
        );
        ...
        let response: CFSearchResult = response
            .json()
            .await
            .map_err(|e| anyhow!("CurseForge search JSON decode error: {}. URL: {}", e, url))?;

        let hits = response
            .data
            .into_iter()
            .map(|item| ResourceProject {
    ...
    async fn get_project(&self, id: &str) -> Result<ResourceProject> {
        ...
        if !response.status().is_success() {
            ...
            let body = response.text().await.unwrap_or_default();
            return Err(anyhow!(
                "CurseForge API error fetching project ({}): {}",
                status,
                body
            ));
        }
    ...
    async fn get_projects(&self, ids: &[String]) -> Result<Vec<ResourceProject>> {
        ...
        if !response.status().is_success() {
            return Err(anyhow!(
                "CurseForge batch project fetch failed: {}",
                response.status()
            ));
        }
    ...
    async fn get_versions(
        ...
        for page_idx in 0..20 {
            ...
            if !response.status().is_success() {
                ...
                let body = response.text().await.unwrap_or_default();
                return Err(anyhow!(
                    "CurseForge API error fetching versions ({}): {}",
                    status,
                    body
                ));
            }
            ...
            let res_data: CFFilesResponse = response.json().await.map_err(|e| {
                anyhow!(
                    "CurseForge versions JSON decode error: {}. Project: {}",
                    e,
                    project_id
                )
            })?;
    ...
    async fn get_version(&self, project_id: &str, version_id: &str) -> Result<ResourceVersion> {
        ...
        if !response.status().is_success() {
            return Err(anyhow!(
                "CurseForge version fetch failed: {}. URL: {}",
                response.status(),
                url
            ));
        }
    ...
    async fn get_by_hash(&self, hash: &str) -> Result<(ResourceProject, ResourceVersion)> {
        let fingerprint = hash
            .parse::<u32>()
            .map_err(|_| anyhow!("Invalid fingerprint: {}", hash))?;
        ...
        let result: CFFingerprintResponse = response.json().await.map_err(|e| {
            anyhow!(
                "CurseForge fingerprint JSON decode error: {}. Hash: {}",
                e,
                hash
            )
        })?;

        let match_item = result
            .data
            .exact_matches
            .first()
