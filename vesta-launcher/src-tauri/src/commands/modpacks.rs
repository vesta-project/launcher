...
pub async fn get_modpack_info(
    ...
    let metadata = get_modpack_metadata(&path_buf)
        ...
        let result = match platform {
            SourcePlatform::CurseForge => {
                ...
                if let Ok(fp) = calculate_curseforge_fingerprint(&path_buf) {
                    log::info!("[get_modpack_info] CurseForge fingerprint calculated: {}", fp);
                    found = resource_manager.get_by_hash(*platform, &fp.to_string()).await.ok();
                }
                
                // Try raw Murmur2 if first one fails
                if found.is_none() {
                    if let Ok(raw_fp) = calculate_murmur2_raw(&path_buf) {
                        log::info!("[get_modpack_info] CurseForge RAW Murmur2 calculated: {}", raw_fp);
                        found = resource_manager.get_by_hash(*platform, &raw_fp.to_string()).await.ok();
                    }
                }
                found
            }
    ...
    for platform in platforms {
        log::info!("[get_modpack_info] Searching for modpack '{}' on {:?}", info.name, platform);
        ...
        if let Ok(response) = resource_manager.search(platform, query).await {
            log::info!("[get_modpack_info] Found {} potential matches on {:?}", response.hits.len(), platform);
            ...
            for hit in response.hits {
                ...
                log::debug!("[get_modpack_info] [{:?}] Score {} for hit '{}' by '{}' ({})", platform, score, hit.name, hit.author, hit.id);
            ...
            if let Some(hit) = best_hit {
                ...
                log::info!("[get_modpack_info] Picked best match: '{}' ({}) with score {}", hit.name, hit.id, max_score);
                ...
                log::info!("[get_modpack_info] Searching for version '{}' on platform {:?} (ID: {})", 
                    info.version, platform, hit.id);

                // Try to find matching version ID
                if let Ok(versions) = resource_manager.get_versions(platform, &hit.id, false, Some(&info.minecraft_version), Some(&info.modloader)).await {
                    let search_v = info.version.to_lowercase();
                    
                    if let Some(v) = versions.into_iter().find(|v| {
                        let v_num = v.version_number.to_lowercase();
                        // 1. Exact match
                        if v_num == search_v { return true; }
                        ...
                        if v_num.contains(&search_v) {
                             ...
                             let prev_char = if idx > 0 { v_num.as_bytes()[idx-1] as char } else { ' ' };
                             let next_char = if idx + search_v.len() < v_num.len() { v_num.as_bytes()[idx + search_v.len()] as char } else { ' ' };
                        ...
                        log::info!("[get_modpack_info] Found likely version match on {:?}: {}", platform, info.modpack_version_id.as_ref().unwrap());
                    } else {
                        log::info!("[get_modpack_info] No platform version match found for '{}'. Using manifest version.", info.version);
                    }
                }
                
                log::info!("[get_modpack_info] Linked modpack to {:?} project: {}", platform, hit.id);
                break;
            }
        }
    }
...
async fn resolve_modpack_resource(client: &reqwest::Client, url: &str) -> (String, Option<String>) {
    ...
    if url.contains("modrinth.com/") {
        ...
        if url.contains("api.modrinth.com/v2/version/") {
            target_version_id = url.split("api.modrinth.com/v2/version/").nth(1).and_then(|s| s.split('?').next()).map(|s| s.to_string());
        } else if url.contains("cdn.modrinth.com/data/") {
            target_version_id = url.split("/versions/").nth(1).and_then(|s| s.split('/').next()).map(|s| s.to_string());
            log::info!("[resolve_modpack_resource] Detected direct CDN link, extracted version ID: {:?}", target_version_id);
        } else {
            let pattern = if url.contains("modrinth.com/modpack/") { "modrinth.com/modpack/" } else { "modrinth.com/project/" };
            slug = url.split(pattern)
                .nth(1)
            ...
            if url.contains("/version/") {
                target_version_id = url.split("/version/").nth(1).and_then(|s| s.split('?').next()).map(|s| s.to_string());
            } else if let Some(q) = url.split('?').nth(1) {
                target_version_id = q.split('&').find(|p| p.starts_with("version=")).map(|p| p.replace("version=", ""));
            }
        }
        ...
        let version_json = if let Some(vid) = target_version_id {
            ...
            client.get(&ver_api_url).send().await.ok().and_then(|r| if r.status().is_success() { Some(r) } else { None })
        } else if let Some(slug_str) = slug {
            let versions_url = format!("https://api.modrinth.com/v2/project/{}/version", slug_str);
            client.get(&versions_url).send().await.ok().and_then(|r| if r.status().is_success() { Some(r) } else { None })
        } else {
            None
        };
            ...
            if let Ok(json_val) = resp.json::<serde_json::Value>().await {
                ...
                if let Some(v) = version {
                    ...
                    if let Some(files) = v["files"].as_array() {
                        log::debug!("[resolve_modpack_resource] Version {} has {} files", v["id"].as_str().unwrap_or("?"), files.len());
                        
                        let filtered: Vec<&serde_json::Value> = files.iter().filter(|f| {
                        ...
                        let selected = filtered.iter().find(|f| f["primary"].as_bool() == Some(true))
                        ...
                        if let Some(file) = selected {
                            if let Some(u) = file["url"].as_str() {
                                ...
                                log::info!("[resolve_modpack_resource] Selected file: {} (primary: {:?})", 
                                    file["filename"].as_str().unwrap_or("?"), file["primary"].as_bool());
                            }
                        }
                    }
                }
...
pub async fn get_modpack_info_from_url(
    ...
    if url.contains("modrinth.com/") {
        ...
        if url.contains("api.modrinth.com/v2/version/") {
            version_id = url.split("v2/version/").nth(1).and_then(|s| s.split('?').next()).map(|s| s.to_string());
        } else if url.contains("cdn.modrinth.com/data/") {
            version_id = url.split("/versions/").nth(1).and_then(|s| s.split('/').next()).map(|s| s.to_string());
        } else {
            let pattern = if url.contains("modrinth.com/modpack/") { "modrinth.com/modpack/" } else { "modrinth.com/project/" };
            slug = url.split(pattern)
                .nth(1)
            ...
            if url.contains("/version/") {
                version_id = url.split("/version/").nth(1).and_then(|s| s.split('?').next()).map(|s| s.to_string());
            } else if let Some(q) = url.split('?').nth(1) {
                version_id = q.split('&').find(|p| p.starts_with("version=")).map(|p| p.replace("version=", ""));
            }
        }
                    ...
                    if let Ok(p_resp) = client.get(&p_url).send().await {
                        if let Ok(p) = p_resp.json::<serde_json::Value>().await {
                            return Ok(ModpackInfo {
                                ...
                                description: Some(p["description"].as_str().unwrap_or("").to_string()),
                                version: v["version_number"].as_str().unwrap_or("1.0.0").to_string(),
                                icon_url: p["icon_url"].as_str().map(|s| s.to_string()),
                                author: None,
                                minecraft_version: v["game_versions"].as_array().and_then(|a| a.first()).and_then(|g| g.as_str()).unwrap_or("unknown").to_string(),
                                modloader: v["loaders"].as_array().and_then(|a| a.first()).and_then(|l| l.as_str()).unwrap_or("fabric").to_string(),
    ...
    log::info!("[get_modpack_info_from_url] Falling back to ZIP download. Resolved: {} -> {}", url, final_url);
    
    let response = client.get(&final_url).send().await.map_err(|e| e.to_string())?;
    ...
    while let Some(chunk_result) = stream.next().await {
        ...
        std::io::Write::write_all(&mut temp_file, &chunk).map_err(|e| format!("Write error: {}", e))?;
        downloaded += chunk.len();
        if downloaded % (1024 * 1024) == 0 {
             log::debug!("[get_modpack_info_from_url] Downloaded {} MB...", downloaded / (1024 * 1024));
        }
    }
...
pub async fn install_modpack_from_zip(
    ...
    log::info!("[install_modpack_from_zip] Data: MC={}, Loader={:?}, LoaderVersion={:?}", 
        instance_data.minecraft_version, instance_data.modloader, instance_data.modloader_version);
...
pub async fn install_modpack_from_url(
    ...
    log::info!("[install_modpack_from_url] Resolved URL: {} -> {}", url, final_url);
