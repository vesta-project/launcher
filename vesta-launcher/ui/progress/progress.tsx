...
export const Progress = <T extends ValidComponent = "div">(
  ...
  const value = local.progress === null || local.progress === undefined || local.progress === -1
    ? undefined
  ...
  return (
    ...
    >
      ...
      <ProgressPrimitive.Track class="progress__track">
        <ProgressPrimitive.Fill class={clsx("progress__fill", isIndeterminate && "progress__fill--indeterminate")} />
      </ProgressPrimitive.Track>
      {local.current_step !== undefined && local.total_steps !== undefined && (
        <div class="progress__steps">Step {local.current_step} of {local.total_steps}</div>
      )}
    </ProgressPrimitive.Root>
  );
};
