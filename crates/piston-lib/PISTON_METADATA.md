...
The `notification` hint allows generator authors to attach an optional notification payload to a `LoaderVersionInfo`. This enables the launcher to create/update UI notifications based on metadata. The field is optional and added to the `LoaderVersionInfo` structure.

Example JSON in `LoaderVersionInfo`:
...
```json
{
  ...
  "metadata": null,
  "notification": {
    "client_key": "forge-47-2-0",
    "title": "Forge available",
    "description": "Forge 47.2.0 is available for Minecraft 1.20.1",
    "notification_type": "Immediate",
    "severity": "Info",
    "dismissible": true,
    "progress": null,
    "show_on_completion": false,
    "actions": [{"action_type":"Open","label":"View Release","payload":{"url":"https://files.minecraftforge.net/"}}]
  }
}
```

Consumers (backend/frontend) should handle missing/unknown fields gracefully. When present, the launcher may upsert a persisted notification (if `client_key` provided) or generate a transient toast if `notification_type` is `Immediate`.
