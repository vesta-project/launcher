...
fn test_library_deduplication_with_natives() {
        ...
        libraries: vec![
            ...
            Library {
                ...
                downloads: Some(LibraryDownloads {
                    artifact: Some(Artifact {
                        path: Some(
                            "org/lwjgl/lwjgl/3.3.1/lwjgl-3.3.1-natives-windows.jar".to_string(),
                        ),
    ...
    let lwjgl_main = unified
        .libraries
        .iter()
        .find(|l| l.name == "org.lwjgl:lwjgl:3.3.1");
    let lwjgl_native = unified
        .libraries
        .iter()
        .find(|l| l.name == "org.lwjgl:lwjgl:3.3.1:natives-windows");

    assert!(lwjgl_main.is_some(), "Main LWJGL library should be present");
    assert!(
        lwjgl_native.is_some(),
        "Native LWJGL library should be present"
    );
...
fn test_library_resolution_with_rules() {
    let manifest = VersionManifest {
        ...
        libraries: vec![Library {
            ...
            rules: Some(vec![serde_json::from_value(serde_json::json!({
                "action": "allow",
                "os": { "name": "windows" }
            }))
            .unwrap()]),
...
fn test_arch_rule_matching() {
    ...
    assert_eq!(
        ...
        0,
        "Library with x86 rule should not match on x64 host"
    );

    // This SHOULD match on most modern machines
    let mut manifest_x64 = manifest.clone();
    manifest_x64.libraries[0].rules.as_mut().unwrap()[0] =
    ...
    assert_eq!(
        unified_x64.libraries.len(),
        1,
        "Library with x64 rule should match on x86_64 host"
    );
}
